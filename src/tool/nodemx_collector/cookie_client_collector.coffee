...
COOKIE_CLIENT_CMD_STATISTIC_NS = "ad.cookie_client:cmd_statistic"
...
collect = (cb)->
	FetchClient.fetchAllClientUrls((err, clientUrls)->
		...
		,(clientUrl, _cb)->
			...
			FetchMBean.fetchNodeNS(clientUrl, COOKIE_CLIENT_CMD_STATISTIC_NS, (err, statistic)->
				if statistic
...
collect((err, cmdCounter)->
	...
	for _ip of cmdCounter
		...
		_cmdCountMsg = SlinkGraphite.buildCookieClientCmdCountPath(_ip, _statisticObj["cmd_count"])
		_cmdErrorCountMsg = SlinkGraphite.buildCookieClientCmdErrorCountPath(_ip, _statisticObj["cmd_error_count"])
		if test
